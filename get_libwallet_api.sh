...
git -C $LOKI_DIR config user.name "SevaBit GUI"
git -C $LOKI_DIR config user.email "gui@sevabit.local"
...
for PR in $(git log --format=%B -n 1 | grep -io "requires #[0-9]*" | sed 's/[^0-9]*//g'); do
    ...
    git -C $LOKI_DIR merge --quiet PR-$PR  -m "Merge sevabit PR #$PR"
